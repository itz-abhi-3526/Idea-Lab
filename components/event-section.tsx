...
export function EventSection() {
  ...
  const upcomingEvents = useMemo(
    () => events.filter((e) => new Date(e.start_datetime) >= now),
    [events, now]
  )

  const featuredUpcoming = useMemo(
    () => upcomingEvents.filter((e) => e.is_featured),
    [upcomingEvents]
  )

  const nonFeaturedUpcoming = useMemo(
    () => upcomingEvents.filter((e) => !e.is_featured),
    [upcomingEvents]
  )
    ...
    <section className="relative w-full py-20 bg-background/50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
        ...
        <div className="mb-16 text-center space-y-4">
          ...
          <div className="h-[2px] w-28 mx-auto bg-gradient-to-r from-accent/0 via-accent to-accent/0" />
