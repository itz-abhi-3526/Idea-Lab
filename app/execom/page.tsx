...
export default function ExecomPage() {
  ...
  useEffect(() => {
    async function init() {
      // 1. Start the visual progress simulation
      ...
      // 3. Brief delay at 100% for that "Tactical Feel"
      setTimeout(() => {
        setIsReady(true)
      }, 2200)
      ...
      <AnimatePresence mode="wait">
        {!isReady ? (
          ...
          >
            ...
            {/* 3. Blueprint Grid with Radial Mask */}
            ...
            <main className="relative z-10 mx-auto max-w-7xl px-6 py-24">
              ...
              <header className="relative mb-32 border-l-2 border-orange-600 pl-8">
                ...
                <h1 className="flex flex-wrap items-baseline gap-x-4 text-[12vw] md:text-[11rem] font-black uppercase leading-[0.8] tracking-tighter italic whitespace-nowrap">
                ...
                <div className="mt-8 flex flex-wrap gap-8 font-mono text-[9px] text-neutral-500 uppercase tracking-[0.3em]">
                  <span className="flex items-center gap-2"><div className="h-1 w-1 bg-green-500" /> STATUS: OPERATIONAL</span>
                  <span className="flex items-center gap-2"><div className="h-1 w-1 bg-neutral-700" /> ACCESS: GRANTED</span>
                  <span className="flex items-center gap-2 animate-pulse"><div className="h-1 w-1 bg-orange-600" /> SYNC_LIVE</span>
                </div>
              </header>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                {members.map((member, index) => (
                  <motion.div 
                    ...
                    className="group relative bg-[#0a0a0a] border border-white/5 p-10 transition-all duration-700 hover:border-orange-600/60 hover:shadow-[0_0_40px_-15px_rgba(234,88,12,0.2)]"
                  >
                    <div className="flex justify-between items-start mb-12">
                      <div className="h-32 w-32 bg-[#111] border border-white/10 overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-700 relative">
                        {member.image_url && (
                          <>
                            <img src={member.image_url} alt="" className="h-full w-full object-cover brightness-75 group-hover:brightness-100 transition-all duration-700 scale-110 group-hover:scale-100" />
